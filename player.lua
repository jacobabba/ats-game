do
    ...
    function p:update(kl, w)
        ...
        if kl.jump.pressed and kl.jump.frame == 1 and self.jump then
            --jump
        ...
        if kl.left.pressed and not kl.right.pressed then
            self.vx = math.max(-self.MAX_HSPEED, self.vx - self.HACCEL)
        elseif kl.right.pressed and not kl.left.pressed then
        ...
        if cty < 1 then
            if self.vy > 0 then self.jump = true
            else self.jump = false end
            self.vy = 0
        else
            self.jump = false
        end
