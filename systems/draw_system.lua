        ...
        for _,v in ipairs(entities) do
            ...
            elseif v.drawable.drawType == "line" then
                ...
                for _,segv in ipairs(v.line.segments) do
                    if segv.width then
                        love.graphics.line(
                            math.floor((segv.x-1)*TILE_SIZE+world.shiftX),
                            math.floor((segv.y-.5)*TILE_SIZE+world.shiftY),
                            math.floor((segv.x-1+segv.width)*TILE_SIZE+world.shiftX),
                            math.floor((segv.y-.5)*TILE_SIZE+world.shiftY)
                        )
                    elseif segv.height then
                        love.graphics.line(
                            math.floor((segv.x-.5)*TILE_SIZE+world.shiftX),
                            math.floor((segv.y-1)*TILE_SIZE+world.shiftY),
                            math.floor((segv.x-.5)*TILE_SIZE+world.shiftX),
                            math.floor((segv.y-1+segv.height)*TILE_SIZE+world.shiftY)
                        )
                    end
                end
            end
